...
def calculate_h2h_metrics(h2h_data, league_name):
    ...
    metrics = {
        'player1_win_percentage': h2h_data.get('player1_win_percentage', 0) or 0,
        'player2_win_percentage': h2h_data.get('player2_win_percentage', 0) or 0,
        'over_0_5_ht_percentage': (over_0_5_ht / total) * 100.0,
        'over_1_5_ht_percentage': (over_1_5_ht / total) * 100.0,
        'over_2_5_ht_percentage': (over_2_5_ht / total) * 100.0,
        'btts_ht_percentage': (btts_ht / total) * 100.0
    }
    print(
        "[DEBUG] H2H %s | O0.5=%.1f O1.5=%.1f O2.5=%.1f BTTS=%.1f"
        % (league_name, metrics['over_0_5_ht_percentage'], metrics['over_1_5_ht_percentage'],
           metrics['over_2_5_ht_percentage'], metrics['btts_ht_percentage'])
    )
    return metrics
...
def format_message(match, h2h_metrics, strategy, bet365_ev_id):
    ...
    if h2h_metrics:
        msg += (
            ...
            f"⚽ +0.5 Gols HT: <i>{h2h_metrics.get('over_0_5_ht_percentage', 0):.2f}%</i>\n"
            f"⚽ +1.5 Gols HT: <i>{h2h_metrics.get('over_1_5_ht_percentage', 0):.2f}%</i>\n"
            f"⚽ +2.5 Gols HT: <i>{h2h_metrics.get('over_2_5_ht_percentage', 0):.2f}%</i>\n"
            ...
            for tip in sent_tips:
                ...
                if tip['status'] == 'pending':
                    ...
                    if m:
                        ...
                        print(f"[DEBUG] Tip {tip['match_id']} ⇒ {tip['status']} (HT gols={ht_goals})")
                        ...
                        if tip['status'] in ['green', 'red']:
                            ...
                            try:
                                await bot.edit_message_text(chat_id=CHAT_ID, message_id=tip['message_id'], text=new_text, parse_mode="HTML", disable_web_page_preview=True)
            ...
            for match in matches:
                ...
                if (home_goals == 1 and away_goals == 0) or (home_goals == 0 and away_goals == 1):
                    ...
                    # 12/10 mins play: até 5'
                        ...
                        print(f"[DEBUG] {match_id}: +1.5 HT NÃO — t={current_time:.2f} O1.5={h2h_metrics.get('over_1_5_ht_percentage','N/A') if h2h_metrics else 'N/A'} O2.5={h2h_metrics.get('over_2_5_ht_percentage','N/A') if h2h_metrics else 'N/A'}")
                    continue  # já decidiu para +1.5, não avalia +0.5

                # ---------- Estratégia +0.5 HT: 0x0 ----------
                if total_goals != 0:
                    print(f"[DEBUG] {match_id}: +0.5 HT NÃO — placar ≠ 0-0")
                    continue

                if current_time <= 0:
                    print(f"[DEBUG] {match_id}: +0.5 HT NÃO — tempo=0")
                    continue
                ...
                if "Esoccer Battle - 8 mins play" in league_name:
                    ...
                    if (h2h_metrics
                        ...
                        print(f"[DEBUG] {match_id}: +0.5 HT (Battle 8m) NÃO — O1.5={h2h_metrics.get('over_1_5_ht_percentage','N/A') if h2h_metrics else 'N/A'} BTTS={h2h_metrics.get('btts_ht_percentage','N/A') if h2h_metrics else 'N/A'} t={current_time:.2f}")
                else:
                    # Outras ligas — incluir taxa de ataques perigosos
                    da_rate = calculate_dangerous_attacks_rate(match, current_time)
                    if da_rate < 1.0:
                        print(f"[DEBUG] {match_id}: +0.5 HT NÃO — da_rate {da_rate:.2f} < 1.0")
                        continue

                    if (h2h_metrics
                        and h2h_metrics.get('over_0_5_ht_percentage', 0) >= 100.0
                        and h2h_metrics.get('over_1_5_ht_percentage', 0) >= 85.0):
                        print(f"[DEBUG] {match_id}: +0.5 HT OK — da_rate={da_rate:.2f}")
                        msg = format_message(match, h2h_metrics, "+0.5 HT", bet365_ev_id)
                        await send_message(bot, match_id, msg, sent_matches, "+0.5 HT")
                    else:
                        print(f"[DEBUG] {match_id}: +0.5 HT NÃO — O0.5={h2h_metrics.get('over_0_5_ht_percentage','N/A') if h2h_metrics else 'N/A'} O1.5={h2h_metrics.get('over_1_5_ht_percentage','N/A') if h2h_metrics else 'N/A'} da_rate={da_rate:.2f}")
